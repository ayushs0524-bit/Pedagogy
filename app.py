...
import bhashini
bhashini_configured = bool(st.secrets.get("BHASHINI_USER_ID"))
...
st.markdown(
    ...
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@500;600;700&family=Noto+Sans:wght@400;500;700&family=Noto+Sans+Devanagari:wght@400;500;700&display=swap" rel="stylesheet">
    <style>
    ...
    /* App background + base font, Mac-friendly light theme */
    .stApp {
    ...
    section[data-testid="stSidebar"] * { color: #EDEAE0 !important; }
    ...
    /* Cards for Hindi input / Santali output, echoing the app mockup */
    .mic-card {
        background: var(--slate);
        color: #fff;
        border-radius: 16px;
    ...
    /* Tabs styled like the language pills in the mockup */
    ...
    h1, h2, h3 { color: var(--slate) !important; font-family: 'Poppins', sans-serif; }
...
st.markdown(
    '<div class="main-title">Sahayak · Vernacular Maths Assistant</div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    ...
    <br>
    Class 3 · Mathematics · Jharkhand pilot
    </div>
    """,
    unsafe_allow_html=True,
)
...
if generate_button:
    ...
    if not question.strip():
        ...
        if audio_output:
            ...
            st.caption(
                "🔊 Hindi voice output. Sarvam Bulbul v3 does not "
                "provide a Santali voice yet."
            )
        ...
        # ----------------------------------------------------
        # STEP 5b: SANTALI VOICE OUTPUT (via Bhashini)
        # ----------------------------------------------------

        if bhashini_configured:

            st.subheader("🔊 Listen to Santali Translation")

            with st.spinner("🔊 Generating Santali voice via Bhashini..."):

                try:
                    santali_audio = bhashini.santali_text_to_speech(
                        santali_translation
                    )
                    st.audio(santali_audio, format="audio/wav")
                    st.caption("🔊 Santali voice output, via Bhashini TTS.")

                except Exception as e:
                    st.warning(
                        f"Bhashini Santali voice generation failed: {e}"
                    )

        else:

            st.caption(
                "Add BHASHINI_USER_ID and BHASHINI_API_KEY to "
                ".streamlit/secrets.toml to enable real Santali voice output."
            )
